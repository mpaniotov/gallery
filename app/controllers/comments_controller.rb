class CommentsController < ApplicationController
  ...
  def create
    ...
    if simple_captcha_valid?
      @comment.save
      Action.create(:user_id=>current_user.id, :action_type=>"comments", :data => {:url=>request.original_url+'#'+@comment.id.to_s,:id=>@comment.id,:description=>'Left comment',:time=>Time.zone.now})
    end
